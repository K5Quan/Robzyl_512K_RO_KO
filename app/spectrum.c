    ...
    if (isFirst) {
     ...
     if (settings.rssiTriggerLevelUp == 50) {
        if  (rssi > previousRssi + UOO_trigger)  {
            ...
            lastReceivingFreq = scanInfo.f; // Запоминаем, что мы ее уже посчитали
        }
    } else {
            ...
            scanInfo.rssi = GetRssi(); //test off
            }
    } 
...
/*
  static uint8_t z = 5; // 5 to 3
  int newDbMax;
    if (scanInfo.rssiMax > 0) {
        newDbMax = clamp(Rssi2DBm(scanInfo.rssiMax), -120, 0);
    ...
    if (scanInfo.rssiMin > 0) { 
        settings.dbMin = clamp(Rssi2DBm(scanInfo.rssiMin), -150, -130);
...
{LoadSettings();
   ...
   // uint32_t newFreq = gTxVfo->pRX->Frequency;
